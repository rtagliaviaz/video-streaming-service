    ...
    return (
        ...
        >
                    ...
                    {video.qualities && video.qualities.length > 0 && (
                        <span>
                            {video.qualities.map(q => (
                                <span key={q} className="quality-badge" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                <span className={`status ${video.exists ? 'ready' : 'missing'}`} style={{
                    fontSize: '0.7rem',
                    fontWeight: '600',
                    padding: '0.15rem 0.6rem',
                    borderRadius: '9999px',
                    background: video.exists ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)',
                    color: video.exists ? '#10b981' : '#ef4444',
                }}>
                    {video.exists ? '✅ Ready' : '❌ Missing'}
                </span>
                {video.exists && (
                    <button
                        className="delete"
