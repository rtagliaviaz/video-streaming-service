...
export const useVideoUpload = ({ onUploadSuccess }: UseVideoUploadProps) => {
    ...
    const [isComplete, setIsComplete] = useState(false); // ✅ NUEVO

    const { progress: processingProgress, isComplete: sseComplete, error: sseError } = useSSE(jobId);
    ...
    useEffect(() => {
        if (sseComplete && videoId) {
            ...
            // no limpiar jobId/videoId para mantener el estado "completado"
        }
    ...
    const uploadFile = useCallback(async () => {
        ...
        try {
            const response = await videoApi.uploadVideo(file, (percent) => {
            ...
            console.log(`📹 Video ID: ${vid}, Job ID: ${jid}`);
